...
public class debatecoach {
    ...
    // Voice Argue mode: speaks the AI's replies aloud via Groq TTS.
    // Note: playai-tts was decommissioned by Groq - orpheus is the current model.
    static final String TTS_MODEL = "canopylabs/orpheus-v1-english";
    static final String TTS_VOICE = "austin";
    ...
    public static void main(String[] args) throws Exception {
        ...
        server.createContext("/speak", debatecoach::handleSpeak);
    ...
    static void handleArgue(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            String persona = extractField(requestBody, "persona");

            String reply = getArgueReply(history, persona);
    ...
    // ---------- Voice Argue: text-to-speech for AI replies ----------

    static void handleSpeak(HttpExchange exchange) throws IOException {
        exchange.getResponseHeaders().add("Access-Control-Allow-Origin", "*");
        exchange.getResponseHeaders().add("Access-Control-Allow-Methods", "POST, OPTIONS");
        exchange.getResponseHeaders().add("Access-Control-Allow-Headers", "Content-Type");
        ...
        try {
            String requestBody = new String(exchange.getRequestBody().readAllBytes(), StandardCharsets.UTF_8);
            String text = extractField(requestBody, "text");
            if (text.isEmpty()) {
                throw new RuntimeException("No text provided");
            }
            String persona = extractField(requestBody, "persona");

            byte[] audioBytes = getSpeechFromGroq(text, persona);
    ...
    // Calls Groq's TTS endpoint and returns raw WAV audio bytes.
    static byte[] getSpeechFromGroq(String text, String persona) throws Exception {
        if (GROQ_API_KEY == null || GROQ_API_KEY.isEmpty()) {
            throw new RuntimeException("GROQ_API_KEY environment variable is not set!");
        }

        // Defensive cap - TTS providers can choke on very long input, and
        // debate replies are already kept under ~130 words by the prompt.
        String safeText = text.length() > 1500 ? text.substring(0, 1500) : text;
        String voice = personaVoice(persona);
        ...
        String jsonPayload = """
            ...
            """.formatted(TTS_MODEL, voice, escapeJson(safeText));
        ...
        if (response.statusCode() != 200) {
            throw new RuntimeException("Groq TTS API Error: " + new String(response.body(), StandardCharsets.UTF_8));
        }

        return response.body();
    }

    // Generic single-field JSON string extractor (handles escaped quotes,
    // backslashes, and unicode escapes within the value).
    ...
    // ---------- Debate personas: shared by /argue (prompt) and /speak (voice) ----------
    // Voices are Groq Orpheus English voices: autumn, diana, hannah, austin, daniel, troy.
    static String personaVoice(String persona) {
        if (persona == null) return TTS_VOICE;
        switch (persona) {
            case "skeptic":    return "troy";
            case "idealist":   return "hannah";
            case "pragmatist": return "daniel";
            default:           return TTS_VOICE;
        }
    }

    static String personaSystemPrompt(String persona) {
        String base =
        ...
        switch (persona) {
            case "skeptic":
                return base + " Your persona is The Skeptic: relentlessly probe for hidden "
                  + "assumptions, demand evidence for every claim, and call out when the "
                  + "user is generalizing from limited examples. Tone: incisive, terse, "
                  + "unimpressed by rhetoric.";
            case "idealist":
                return base + " Your persona is The Idealist: argue from first principles "
                  + "and values, appeal to what the world should look like, and push back "
                  + "when the user's position trades away a principle for convenience. "
                  + "Tone: warm, passionate, morally engaged.";
            case "pragmatist":
                return base + " Your persona is The Pragmatist: focus on real-world "
                  + "tradeoffs, costs, incentives, and what has actually worked in "
                  + "practice. Tone: calm, measured, skeptical of idealism with no "
                  + "implementation plan.";
            default:
                return base;
        }
    ...
    static String getArgueReply(List<String[]> history, String persona) throws Exception {
        if (GROQ_API_KEY == null || GROQ_API_KEY.isEmpty()) {
            throw new RuntimeException("GROQ_API_KEY environment variable is not set!");
        }

        String systemPrompt = personaSystemPrompt(persona);
    ...
    // Decodes JSON string escapes (\n \t \" \\ \/ and unicode escapes) in an already
